		...
		<div className="w-full max-w-3xl">
			...
			{tab === 'daily' ? (
				<table className="w-full border-collapse">
					<thead>
						<tr className="border-b border-neutral-800">
							<th className="px-1 py-1 text-left">Tarih</th>
							<th className="px-1 py-1 text-right">Gelir</th>
							<th className="px-1 py-1 text-right">Gider</th>
					...
					<tbody>
						{daily.map((r) => (
							<tr key={r.date} className="border-b border-neutral-900">
								<td className="px-1 py-1">{r.date}</td>
								<td className="px-1 py-1 text-right">{formatAmount(r.income)}</td>
								<td className="px-1 py-1 text-right">{formatAmount(r.expense)}</td>
								<td className={`px-1 py-1 text-right ${r.net >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{formatAmount(r.net)}</td>
							</tr>
						))}
					</tbody>
				</table>
				...
				<table className="w-full border-collapse">
					<thead>
						<tr className="border-b border-neutral-800">
							<th className="px-1 py-1 text-left">Ay</th>
							<th className="px-1 py-1 text-right">Gelir</th>
							<th className="px-1 py-1 text-right">Gider</th>
							<th className="px-1 py-1 text-right">Net</th>
						</tr>
					...
					<tbody>
						{monthly.map((r) => (
							<tr key={r.month} className="border-b border-neutral-900">
								<td className="px-1 py-1">{r.month}</td>
								<td className="px-1 py-1 text-right">{formatAmount(r.income)}</td>
								<td className="px-1 py-1 text-right">{formatAmount(r.expense)}</td>
								<td className={`px-1 py-1 text-right ${r.net >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{formatAmount(r.net)}</td>
							</tr>
